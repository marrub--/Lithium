...
void DrawAttr(struct gui_state *g, i32 x, i32 y, i32 at)
{
   ...
   PrintTextA_str(ns(language_fmt(LANG "ATTR_HELP_%.3s", name)), sf_smallfnt, g->defcr, g->ox+x+1,1, g->oy+y,1, helptrns);
...
void P_CBI_TabStatus(struct gui_state *g)
{
   ...
   StatusInfo(g, y, ns(language(sl_status_cl)), pl.classname);
   y += 10;

   StatusInfo(g, y, ns(language(sl_status_lv)), StrParam("%u", pl.attr.level));
   y += 10;
   StatusInfo(g, y, ns(language(sl_status_hp)), StrParam("%i/%i", pl.health, pl.maxhealth));
   y += 10;

   if(pl.pclass & pcl_magicuser) {
      StatusInfo(g, y, ns(language(sl_status_mp)), StrParam("%i/%i", pl.mana, pl.manamax));
      y += 10;
   }

   StatusInfo(g, y, ns(language(sl_status_xp)),  StrParam("%u", pl.attr.exp));
   y += 10;
   StatusInfo(g, y, ns(language(sl_status_nx)), StrParam("%u", pl.attr.expnext));
   y += 10;

   y += pl.pclass & pcl_magicuser ? 20 : 30;

   if(pl.attr.points) {
      PrintTextFmt(tmpstr(language(sl_status_levelup)), pl.attr.points);
