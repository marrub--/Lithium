...
void G_ScrBeg_Imp(struct gui_state *g, gid_t id, struct gui_arg_scr *a) {
   ...
   /* positions */
   i32 const orx = a->x + pre->scrlw, ory = a->y;
   i32 const x   = orx + g->ox,       y   = ory + g->oy;
   /* check collision - height is minus caps, and y is offset by the top cap */
   G_Auto(g, id, orx - pre->scrlw, ory + caph, pre->scrlw, h);
   ...
   __with(i32 vofs = 0; /* offset in pixels of the content */) {
      ...
      scr->ox = orx;        /* offset by scrollbar width */
      scr->oy = ory - vofs; /* offset by scroller pos */
   ...
   __with(char scrl[64];) {
      ...
      for(i32 i = 0; i < blocks; i++) {
         PrintSprite(scrls, x,2, y+yofs,1);
         yofs += blockh;
      }
   }
   /* draw bottom cap */
