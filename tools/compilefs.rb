   ...
   for ln in lns
      ...
      if arg
         ...
         when "put file"
            ...
            txt << "\"#{out}\" =\n"
            txt << comment(" #{fnam}")
            txt << buf_lines(:just, open("#{lnc}/#{fnam}", "rt").read.chomp.lines)
         when "parse file"
            txt << parse_file(lnc, open("#{lnc}/#{arg}", "rt").read.chomp.lines)
         end
      end
   end
   ...
   loop do
      ...
      if arg and type == "in"
         ...
         when "file"
            ...
            for l in lng
               larg = arg.sub "*", l[:lnc]
               ths << Thread.start(l[:lnc], l[:lng], "#{dir}/#{larg}", txt) do |lnc, lng, nam, txt|
                  proc_file lnc, lng, txt, nam
               end
            end
         end
      end
